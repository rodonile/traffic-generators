...
function master(args)
	...
	rxDev = device.config{port = args.rxDev, rxQueues = 4, txQueues = 4, rssQueues = 4}
	device.waitForLinks()
    ...
    -- TIMESTAMP PACKETS CANNOT BE CAPTURED, BECAUSE OTHERWISE MEASUREMENT DOESN'T SUCCEDE (issue of dumper interfering with timestampe retrieval  from NIC register)
    lm.startTask("dumper", rxDev:getRxQueue(1), args, 1)
    ...
    mg.startTask("timerSlave", txDev:getTxQueue(1), rxDev:getRxQueue(1), args.size, args.flows, args.time)
    
    --------------------------------------
    --Try to use manually implementen timer slave function (quella già presente è meglio)
    -- Idea: measure latency between both queues every 0.01 seconds. Packets take round trip for measurement to be successful
    --------------------------------------
    --mg.startTask("timerTask", txDev:getTxQueue(1), rxDev:getRxQueue(1), args.size)
    --------------------------------------
...
------------------------- MANUALLY IMPLEMENTED TIMER SLAVE FUNCTION (TAKEN FROM MOONGEN TUTORIAL)
...
function timerTask(txq, rxq, size)
    ...
    hist:print()
    hist:save("histogram_tutorial_udp.csv")
end
