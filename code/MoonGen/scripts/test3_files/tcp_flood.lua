...
function configure(parser)
	...
	parser:option("-r --rate", "Transmit rate in Mbit/s."):default(100):convert(tonumber)
	parser:option("-i --ip", "Source IP (IPv4 or IPv6)."):default("10.0.0.1")
	parser:option("-d --destination", "Destination IP (IPv4 or IPv6)."):default("10.0.0.253")
	parser:option("-f --flows", "Number of different IPs to use."):default(1000):convert(tonumber)
end
