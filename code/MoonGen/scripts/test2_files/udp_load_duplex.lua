...
function master(args)
	...
	rxDev = device.config{port = args.rxDev, rxQueues = 6, txQueues = 6}
	device.waitForLinks()
	-- max 1kpps timestamping traffic timestamping
	-- rate will be somewhat off for high-latency links at low rates
    args_rate_adapted = args.rate - (args.size + 4) * 8 / 1000
    
    txDev:getTxQueue(0):setRate(args_rate_adapted/3)
    txDev:getTxQueue(1):setRate(args_rate_adapted/3)
    txDev:getTxQueue(4):setRate(args_rate_adapted/3)
    
    rxDev:getTxQueue(0):setRate(args_rate_adapted/3)
    rxDev:getTxQueue(1):setRate(args_rate_adapted/3)
    rxDev:getTxQueue(4):setRate(args_rate_adapted/3)
    ...
    mg.startTask("loadSlave", rxDev:getTxQueue(0), txDev, args.size, args.flows, args.time)
    mg.startTask("loadSlave", rxDev:getTxQueue(1), txDev, args.size, args.flows, args.time)
    mg.startTask("loadSlave", rxDev:getTxQueue(4), txDev, args.size, args.flows, args.time)
